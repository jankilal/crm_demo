...
/*---------User End-------------*/
    ...
    public function checkLicence()
    {
		...
		$check_licence = $this->comman_model->getData('tbl_app_activation' , array('licence_key'=>$licence_key));

		if(!empty($check_licence))
		{
			$check_device_id = $this->comman_model->check_by(array('device_id'=> '','licence_key'=>$licence_key),'tbl_app_activation');
			if(empty($check_device_id))
			{
				$checkdouble_device_id = $this->comman_model->check_by(array('device_id'=> $device_id,'licence_key'=>$licence_key),'tbl_app_activation');
			...
			else
			{
				...
				$add_activation = $this->comman_model->updateData('tbl_app_activation', array('id'=>$check_licence[0]->id), $post);
