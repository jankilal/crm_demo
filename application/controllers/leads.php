...
class Leads extends MY_Controller 
{
				...
				if($leads_id == '')
				{
					...
					if($lead_res)
					{
						if(login_role == 3)
						{
							...
							$as_post['lead_id'] 	= $post['lead_id'];
							$as_post['user_id'] 	= login_user;
							$as_post['assign_date_time'] = date('Y-m-d H:i:s');
							$as_post['assign_by'] = login_user;
							$add_res = $this->comman_model->addData('tbl_assign_user' , $as_post);
							if($add_res)
							{
								$this->comman_model->updateData('tbl_leads' , array('id' => $lead_res) , array('lead_assign_status' => '1'));
							}
						}
	...
	public function viewlead($leads_id = '')
	{
		...
		{	
			...
			$this->data['quotation_detail'] = $this->comman_model->getData('tbl_quotation', array('lead_id' => $leads_id));
	...
	public function viewQuotationDetails($quote_id = '' , $back_action1 = '' , $back_action2 = '')
	{
		if($this->checkViewPermission())
		{	
			$this->data['back_action']		= 'opportunities/'.$back_action1.'/'.$back_action2;
